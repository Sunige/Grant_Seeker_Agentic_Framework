...
For European grants, the framework integrates directly with the authoritative **EU Funding & Tenders Portal REST API**. Rather than attempting to scrape CORDIS—which primarily indexes *already funded and active* projects—the pipeline intercepts opportunities upstream at the Funding & Tenders platform when they are in their *Open* or *Forthcoming* statuses. It translates strategic NCC search phrases into API payloads to pull JSON descriptors, allowing the system to flag early-stage Horizon Europe topics before they ever hit the CORDIS registry.
